...
//  Listener para el flujo complejo que enviara cada elemento del objeto    
//  dataComplex como velocidad a arduino.
    public class listenerComplex implements ActionListener {
        @Override
        public void actionPerformed(ActionEvent evt){
            
//  Si el índice indexComplex desborda del tamaño del objeto dataComplex, para el proceso
            if (indexComplex > dataComplex.size()) {
                
                stop();
                
            }
//  Si no, envía la velocidad del índice que le corresponde y pasa al siguiente
//  índice.          
            else{                
...
//  Listener para el timer que controla la corrección del muelle
...
// Función que activa el modo de trabajo local    
    private void enablelocal(){

// Envía la orden a arduino       
        ino.sendLocal(demo);
// Desactiva la posibilidad de modificar o utilizar objetos de la interfaz        
...
// Función que activa el modo de trabajo remoto     
    private void disablelocal(){
                
// Envía la orden a arduino
        ino.sendRemote(demo);
// Activa la posibilidad de modificar o utilizar objetos de la interfaz         
...
// Función que inicia la descarga de líquido      
    private void start(){
...
// Función que detiene la carga o descarga de líquido
    public void stop (){
        
        if (working.getFlag()){
        
            //Detiene el timer que controla la barra de progreso
            timerProgress.stop();
            timerComplex.stop();
            working.setFlag(false);
            jToggleButtonSTOP.setSelected(true);            
            //Envía la orden de detener el motor
            ino.sendStop(demo);
            
        }
    }    
    
// Función que inicia la carga de líquido  
    private void reload(){
